...
class EarthRanger_Sighting_Create_Form(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        endpoint = f"{settings.ER_MAIN}events?event_type={settings.ER_EVENT_TYPE}&state=new&state=active&page_size={NUM_EVENTS}"  # noqa
        headers = {"Authorization": f"Bearer {settings.ER_TOKEN}"}
        rsp = requests.get(endpoint, headers=headers)

        earthranger_serials = EarthRanger_Sighting.objects.values_list("earthranger_serial", flat=True)

        events = [event for event in rsp.json()["data"]["results"] if event["serial_number"] not in earthranger_serials]
        ...
        self.fields["earthranger_event"] = forms.ChoiceField(
            choices=[
                (
                    json.dumps(event),
                    f'Serial: {event["serial_number"]}, '
                    f'Reported by: { event["reported_by"]["username"] if event["reported_by"] and "username" in event["reported_by"] else "Unknown"}',  # noqa
                )
                for event in events
            ],
...
def region_choices():
    region_names = Group_Sighting.objects.values_list("json__event_details__RegionName", flat=True).distinct()
    choices = [(None, "--------------------")]
    for region in region_names:
        if region:
            choices.append((region, region.upper()))
...
class Search_Form(forms.ModelForm):
    ...
    individual_sighting = forms.ModelChoiceField(
        queryset=Individual_Sighting.objects.order_by("pk"), required=False
    )  # .filter(scoring__isnull=False)
...
class Musth_Status_Form(forms.ModelForm):
    class Meta:
        model = Musth_Status
        exclude = ("individual_sighting",)
...
class Combine_Individual_Form(forms.Form):
    ...
    All `Individual_Sighting` objects associated with `duplicate` are reassigned to `correct` and `duplicate` is
    deleted.
    """
...
class Individual_Profile_Form(forms.Form):
    ...
    def __init__(self, individual, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["photos"].choices = (
            [(None, "----")]
            + [(f"individual_photo {photo.pk}", str(photo)) for photo in individual.individual_photo_set.all()]
            + [
...
class Photo_Delete_Form(forms.Form):
    photos = forms.MultipleChoiceField(widget=forms.CheckboxSelectMultiple)

    def __init__(self, *args, photos, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["photos"].choices = [(photo.id, photo.name) for photo in photos]

    def save(self, commit=True):
        assert commit
        Photo.objects.filter(pk__in=self.cleaned_data["photos"]).delete()
