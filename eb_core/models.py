...
auditlog.register(EarthRanger_Sighting)
...
auditlog.register(Subgroup_Sighting)
...
class Seek_Identity(models.Model):
    ...
    # individual_sighting = models.OneToOneField('Individual_Sighting', on_delete=models.CASCADE, null=True, blank=True)

    presence = (('0', 'Absence'), ('1', 'Presence'))

    gender = models.CharField(max_length=1,
                              choices=(('b', 'bull'), ('c', 'cow'), ('?', 'unknown')),
                              null=True,
                              blank=True)

    age = models.CharField(max_length=1,
                           choices=(('6', '1900-1969'), ('7', '1970-1979'), ('8', '1980-1989'), ('9', '1990-1999'),
                                    ('0', '2000-2009'), ('1', '2010-2019'), ('2', '2020-2029')),
                           null=True,
                           blank=True)
    ...
    def __str__(self):
        return ''.join([
            '?' if c is None else c for c in [
                self.gender, self.age, 'T', self.r_tusk, self.l_tusk, 'E', self.r_prom_tear, self.r_prom_hole, self.
                r_sec_tear, self.r_sec_hole, '-', self.l_prom_tear, self.l_prom_hole, self.l_sec_tear, self.l_sec_hole,
                'X', self.r_extreme, self.l_extreme, 'S', self.r_special, self.l_special, self.body_special
            ]
        ])
    ...
    def __array__(self, dtype=None):
        return np.array([
            '?' if c is None else c for c in [
                self.gender, self.age, self.r_tusk, self.l_tusk, self.r_prom_tear, self.r_prom_hole, self.r_sec_tear,
                self.r_sec_hole, self.l_prom_tear, self.l_prom_hole, self.l_sec_tear, self.l_sec_hole, self.r_extreme,
                self.l_extreme, self.r_special, self.l_special, self.body_special
            ]
        ],
                        dtype=dtype)
...
class Elephant_Voices_Identity(models.Model):
    ...
    life_stage_vals = (('0', 'unknown'), ('1', 'senescing adult'), ('2', 'prime adult'), ('3', 'adult'),
                       ('4', 'young adult'), ('5', 'sub-adult'))
    ...
    tusk_attribute_vals = (('0', 'three tusks'), ('1', 'broken left'), ('2', 'broken right'), ('3', 'equal length'),
                           ('4', 'shorter left'), ('5', 'shorter right'), ('6', 'very shorter'), ('7', 'very long'),
                           ('8', 'very thin'), ('9', 'very thick'), ('10', 'very slender'), ('11', 'symmetric'),
                           ('12', 'higher left'), ('13', 'higher right'), ('14', 'up curved'), ('15', 'straight'),
                           ('16', 'splayed'), ('17', 'convergent'), ('18', 'crossed'), ('19', 'skewed'), ('20',
                                                                                                          'wonky'))

    tusk_attribute = MultiSelectField(choices=tusk_attribute_vals, null=True, blank=True)

    ear_condition_vals = (('0', 'completely smooth'), ('1', 'smooth with tiny nicknames'), ('2', 'serrated'),
                          ('3', 'ragged'), ('4', 'wart/bump'), ('5', 'prominent veins'), ('6', 'crinkle'),
                          ('7', 'fold'), ('8', 'damaged'), ('9', 'curtain'), ('10', 'flop'), ('11', 'droopy'))

    ear_condition = MultiSelectField(choices=ear_condition_vals, null=True, blank=True)

    ear_notch_vals = (('0', 'unusual notch'), ('1', 'outstanding notch or tear'), ('2', 'v-notch'), ('3', 'u-notch'),
                      ('4', 'cup-notch'), ('5', 'dip-notch'), ('6', 'scoop-notch'), ('7', 'square-notch'))

    ear_notch = MultiSelectField(choices=ear_notch_vals, null=True, blank=True)

    body_type_vals = (('0', 'collar'), ('1', 'bump/lump right'), ('2', 'bump/lump left'), ('3', 'permanently lame'),
                      ('4', 'head low'))

    body_type = MultiSelectField(choices=body_type_vals, null=True, blank=True)

    trunk_face_features_vals = (('0', 'strange skin pattern'), ('1', 'blind in one eye'), ('2', 'slit cut trunk'),
                                ('3', 'other trunk injury'), ('4', 'wart/bump trunk'), ('5', 'wart/bump face'),
                                ('6', 'wrinkled forehead'), ('7', 'pointed forehead'), ('8', 'chopped trunk'),
                                ('9', 'lip damage'))

    trunk_face_features = MultiSelectField(choices=trunk_face_features_vals, null=True, blank=True)

    tail_features_vals = (('0', 'no tail'), ('1', 'half tail'), ('2', 'kinky tail'), ('3', 'short tail'),
                          ('4', 'no tail hairs'))
...
auditlog.register(Bounding_Box)
...
class Sighting_Bounding_Box(Bounding_Box):
    ...
    def save(self, *args, **kwargs):
        if type(self.photo) is not Sighting_Photo:
            raise ValidationError(f'self.photo is not of type Sighting_Photo')
...
class Individual_Bounding_Box(Bounding_Box):
    ...
    def save(self, *args, **kwargs):
        if type(self.photo) is not Individual_Photo:
            raise ValidationError(f'self.photo is not of type Individual_Photo')
