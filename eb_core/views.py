...
def group_sighting_view(request, earthranger_serial):
    ...
    thumbnails = {
        group_sighting.earthranger_serial: {i: Sighting_Photo.objects.get(image=image['id']).thumbnail.url
    ...
    context = {
        ...
        # Form for creating/modifying `Social_Sighting` objects
        'social_sighting_formset': social_sighting_formset_constructor(group_sighting)(queryset=group_sighting.social_sighting_set.all()),
        # Form for recording unphotographed `Individual` objects
        'unphotographed_individuals_form': Group_Sighting_Unphotographed_Individuals_Form(instance=group_sighting),
    }
...
def group_sighting_edit(request, earthranger_serial):
    ...
    if request.method == 'POST':
        ...
        form = Group_Sighting_Unphotographed_Individuals_Form(
            request.POST, instance=group_sighting)
        if form.is_valid():
            form.save()

        # Social Sightings
        formset = social_sighting_formset_constructor(group_sighting)(request.POST)
        for form in formset:
            if form.is_valid():
                ...
                if 'DELETE' in form.cleaned_data and form.cleaned_data['DELETE']:
                    ...
                    instance.group_sighting = group_sighting
                    instance.save()
                    form.save_m2m()
...
def social_sighting_list(request):
    """View function to list `Social_Sighting` objects."""
    social_sightings = Social_Sighting.objects.all()  # .order_by('-datetime')
    context = {'social_sightings': social_sightings}
    return render(request, 'social_sighting/index.html', context)


def social_sighting_view(request, social_sighting_id):
    """View function to view a `Social_Sighting` object."""
    social_sighting = get_object_or_404(
        Social_Sighting, pk=social_sighting_id)

    bbox_set = [bbox for individual_sighting in social_sighting.individual_sightings.all(
    ) for bbox in individual_sighting.sighting_bounding_box_set.all()]

    images = list({bbox.photo.image.name: {'id': bbox.photo.image.name,
                                           'url': bbox.photo.compressed_image.url,
                                           'full_res': bbox.photo.image.url} for bbox in bbox_set}.values())

    individual_sighting_categories = {k: k.id for i, k in enumerate(
        social_sighting.individual_sightings.all(), 1)}
    ...
    categories = [{'id': i, 'name': f'Individual Sighting {i}'}
                  for i in individual_sighting_categories.values()]

    thumbnails = {
        social_sighting.group_sighting.earthranger_serial: {i: Sighting_Photo.objects.get(image=image['id']).thumbnail.url
                                                            for i, image in enumerate(images)}
    }

    context = {
        # Selected `Social_Sighting`
        'social_sighting': social_sighting,
        # Form for modifying associated `notes`
        'notes_form': Social_Sighting_Notes_Form(instance=social_sighting),
        # Images associated with the `Group_Sighting` object
        'images': json.dumps(images),
        # JSON bounding boxes for the image viewer
        'boxes': json.dumps(boxes),
        # Annotation categories for all associated `Individual_Sighting` objects
        'categories': json.dumps(categories),
        # Thumbnails associated with the `Social_Sighting` object
        'thumbnails': thumbnails,
    }
    return render(request, 'social_sighting/view.html', context)


def social_sighting_edit(request, social_sighting_id):
    """View function to modify a `Social_Sighting` object."""
    social_sighting = get_object_or_404(
        Social_Sighting, pk=social_sighting_id)

    if request.method == 'POST':
        # Notes
        form = Social_Sighting_Notes_Form(
            request.POST, instance=social_sighting)
        if form.is_valid():
            form.save()

    return redirect(social_sighting_view, social_sighting_id=social_sighting.id)
...
def individual_sighting_unidentified(request):
    ...
    individual_sightings = Individual_Sighting.objects.all().exclude(individual__isnull=False).exclude(unidentifiable=True)
    context = {'individual_sightings': individual_sightings}
...
def individual_sighting_view(request, individual_sighting_id):
    ...
    context = {
        ...
        'form': Individual_Sighting_Form(instance=individual_sighting, user=request.user),
    }
...
def individual_sighting_edit(request, individual_sighting_id):
    ...
    if request.method == 'POST':
        ...
        form = Individual_Sighting_Form(request.POST, instance=individual_sighting, user=request.user)
        if form.is_valid():
            form.save()
...
def view_media(request, name):
    """View function to interact with media using ElephantBook tools."""
    context = {
        'url': os.path.join(settings.MEDIA_URL, name),
    }
    return render(request, 'view_media/index.html', context)
